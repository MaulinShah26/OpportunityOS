...
class PublicSourceResearchProvider:
    def __init__(self, timeout_seconds: float = 15.0, max_source_bytes: int = 1_000_000) -> None:
    ...
    def collect(self, source: OpportunityInput) -> list[EvidenceClaim]:
        ...
        if source.raw_text and source.raw_text.strip():
            excerpt = " ".join(source.raw_text.split())[:5000]
            claims.append(
                EvidenceClaim(
                    claim="User-provided opportunity content",
        ...
        if source.source_url:
            ...
            with httpx.Client(timeout=self.timeout_seconds, follow_redirects=True) as client:
                with client.stream(
                    "GET",
                    url,
                    headers={"User-Agent": "OpportunityOS/0.1"},
                ) as response:
            ...
            excerpt = " ".join(parser.parts)[:5000]
            claims.append(
                EvidenceClaim(
                    claim="Content retrieved from the supplied public URL",
                    claim_type=EvidenceType.OBSERVED_FACT,
                    source_url=source.source_url,
                    supporting_excerpt=excerpt or "Page contained no extractable visible text.",
                    confidence=0.85 if excerpt else 0.3,
                )
            )
        return claims
...
class InputOnlyResearchProvider(PublicSourceResearchProvider):
    """Deterministic test provider that never performs network access."""

    def collect(self, source: OpportunityInput) -> list[EvidenceClaim]:
        if not source.raw_text:
            return [
                EvidenceClaim(
                    claim="URL supplied but not fetched in mock mode",
                    claim_type=EvidenceType.OBSERVED_FACT,
                    source_url=source.source_url,
                    supporting_excerpt=str(source.source_url),
                    confidence=0.35,
                )
            ]
        return super().collect(
            OpportunityInput(raw_text=source.raw_text, company_hint=source.company_hint)
        )
