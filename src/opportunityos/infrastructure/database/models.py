...
class MemoryAuditRecord(Timestamped, Base):
    __tablename__ = "memory_audit_events"
    __table_args__ = (Index("ix_memory_audit_user_created", "user_id", "created_at"),)
    user_id: Mapped[str] = mapped_column(String(36), ForeignKey("users.id"))
    memory_item_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("memory_items.id"))
...
class CompanyRecord(Timestamped, Base):
    ...
    company_json: Mapped[dict] = mapped_column(JSON)


class OpportunityRecord(Timestamped, Base):
    __tablename__ = "opportunities"
    __table_args__ = (Index("ix_opportunities_user_status", "user_id", "status"),)
    user_id: Mapped[str] = mapped_column(String(36), ForeignKey("users.id"))
    company_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("companies.id"))
    source_url: Mapped[str | None] = mapped_column(String(2048))
    raw_text: Mapped[str | None] = mapped_column(Text)
    opportunity_json: Mapped[dict] = mapped_column(JSON)
...
class AnalysisRunRecord(Timestamped, Base):
    ...
    user_id: Mapped[str] = mapped_column(String(36), ForeignKey("users.id"))
    opportunity_id: Mapped[str] = mapped_column(String(36), ForeignKey("opportunities.id"))
    orchestrator: Mapped[str] = mapped_column(String(40))
    model_metadata_json: Mapped[dict] = mapped_column(JSON)
...
class BehaviourEventRecord(Timestamped, Base):
    ...
    __table_args__ = (Index("ix_behaviour_user_type", "user_id", "event_type"),)
    user_id: Mapped[str] = mapped_column(String(36), ForeignKey("users.id"))
    analysis_run_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("analysis_runs.id"))
