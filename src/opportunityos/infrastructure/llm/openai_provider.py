    ...
    def _request(
        ...
        *,
        ...
        try:
            ...
            parsed = validator(parse_json_text(response.output_text))
...
class OpenAIOpportunityExtractor(_OpenAIJSONProvider):
    """OpenAI-backed typed extraction with bounded, validated JSON output."""
    ...
    def extract(self, source: OpportunityInput, evidence: list[EvidenceClaim]) -> OpportunityProfile:
        payload = {
            "instruction": (
                "Extract only facts supported by the supplied opportunity text and evidence. "
                "Do not infer missing compensation, location, seniority, responsibilities, or company facts. "
                "Use evidence IDs exactly as supplied. Return only one JSON object matching the schema."
            ),
...
class OpenAIBusinessAnalyst(_OpenAIJSONProvider):
    ...
    def analyse(
        ...
        payload = {
            "instruction": (
                "Return at most three business hypotheses. Observed facts and supported inferences must cite "
                "one or more evidence IDs from the opportunity. Speculation must be labelled speculative and "
                "kept at confidence 0.60 or lower. Never claim internal company knowledge. Return only JSON."
            ),
...
class OpenAIOutreachWriter(_OpenAIJSONProvider):
    def __init__(self, *, max_output_tokens: int, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.max_output_tokens = max_output_tokens
