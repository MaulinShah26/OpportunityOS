...
def test_profile_analysis_and_feedback_are_persisted(
    database: Database,
    strong_profile: PersonalProfile,
) -> None:
    with database.session() as session:
        store = SqlAlchemyStore(session)
        store.save_profile(strong_profile, email="person@example.com")
    ...
    with database.session() as session:
        ...
        updated, changes = apply_feedback(
            store.get_profile(strong_profile.user_id),
            feedback,
    ...
    with database.session() as session:
        store = SqlAlchemyStore(session)
        refreshed = store.get_profile(strong_profile.user_id)
        fractional = next(
            preference
            for preference in refreshed.preferences
            if preference.key == "engagement:fractional"
        )
        assert fractional.weight > 0.5


def test_feedback_rejects_unknown_analysis(
    database: Database,
    strong_profile: PersonalProfile,
) -> None:
    with database.session() as session:
